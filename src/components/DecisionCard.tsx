...
// distance alone meant a fast flick that didn't happen to cross
// SWIPE_THRESHOLD just sprang back, which is what users on Android/iPhone/
// iPad were reporting as the card "not being sensitive enough." So a swipe
// commits if EITHER the drag crossed the distance threshold (a slow,
// deliberate drag) OR it was moving fast enough at release (a flick),
// matching how swipe gestures actually feel.
...
export function DecisionCard({
  ...
  // A MotionValue, not React state: Motion updates the DOM directly from
  // its own render loop as this changes (every drag frame, every animation
  // tick), so dragging never re-renders this component — the same reason
  // the old hand-rolled version used a ref for velocity tracking, just
  // extended to position too.
